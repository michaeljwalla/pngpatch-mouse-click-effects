...
class MouseClickEffects {
	...
	create_icon_data(click_type, color) {
		...
		contents = this._recolorSvgContents(contents, color);
	...
	_recolorSvgContents(contents, color) {
		const blackFillRegex = /fill\s*=\s*(["'])\s*(#000000|#000|rgb\(\s*0\s*,\s*0\s*,\s*0\s*\)|black)\s*\1/gi;
		let replacedAnyFill = false;

		contents = contents.replace(blackFillRegex, (match, quote) => {
			replacedAnyFill = true;
			return `fill=${quote}${color}${quote}`;
		});

		if (!replacedAnyFill && contents.match(/<image\b/i)) {
			contents = this._injectSvgImageTintFilter(contents, color);
		}

		return contents;
	}

	_injectSvgImageTintFilter(contents, color) {
		const filterId = 'mouseClickEffectsTint';
		const defsMarkup = `<defs><filter id="${filterId}"><feFlood flood-color="${color}" result="flood"/><feComposite in="flood" in2="SourceAlpha" operator="in"/></filter></defs>`;

		if (contents.match(/<defs\b/i)) {
			contents = contents.replace(/<defs\b([^>]*)>/i, `<defs$1><filter id="${filterId}"><feFlood flood-color="${color}" result="flood"/><feComposite in="flood" in2="SourceAlpha" operator="in"/></filter>`);
		} else {
			contents = contents.replace(/<svg\b([^>]*)>/i, `<svg$1>${defsMarkup}`);
		}

		contents = contents.replace(/<image\b/gi, `<image filter="url(#${filterId})"`);
